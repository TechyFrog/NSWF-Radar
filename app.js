...
function createMarkerIcon(color, isBase, initials, heading) {
...
function updateMarker(userId, userData) {
    ...
    if (state.markers.has(userId)) {
        ...
        marker.setIcon(createMarkerIcon(color, isBase, initials, userData.heading));
    } else {
        const marker = L.marker([userData.lat, userData.lng], {
            icon: createMarkerIcon(color, isBase, initials, userData.heading)
        }).addTo(map);
...
// Initialize
console.log('Live NSWF Radar v4.1 initialized');
