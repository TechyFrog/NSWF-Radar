        ...
        if (zoom >= 12) {
            const label = L.marker([minLat + gridSize * 0.5, lng], {
                icon: L.divIcon({
                    ...
                    html: `<div style="color: #000000; font-size: 10px; font-weight: bold; background: rgba(255,255,255,0.7); padding: 2px 4px; border-radius: 2px;">${lng.toFixed(3)}</div>`,
                    iconSize: [50, 20]
                })
            });
            labels.push(label);
    ...
    for (let lat = minLat; lat <= maxLat; lat += gridSize) {
        ...
        if (zoom >= 12) {
            const label = L.marker([lat, minLng + gridSize * 0.5], {
                icon: L.divIcon({
                    className: 'grid-label',
                    html: `<div style="color: #000000; font-size: 10px; font-weight: bold; background: rgba(255,255,255,0.7); padding: 2px 4px; border-radius: 2px;">${lat.toFixed(3)}</div>`,
                    iconSize: [50, 20]
                })
            });
            labels.push(label);
...
async function updateNavigationRoute() {
    // Debounce updates (every 5 seconds max)
    ...
    if (route && state.navigationMode) {
        ...
        if (distanceValue) {
            distanceValue.textContent = formatDistance(distanceKm);
        }
    }
}
                ...
                ">
                    ...
                    ${(heading !== null && heading !== undefined) ? `
                        <path d="M20 5 L15 15 L20 12 L25 15 Z" 
                            fill="#fff" 
                            transform="rotate(${Math.round(heading)} 20 20)"/>
                    ` : ''}
                </svg>
                <div style="
                    position: absolute;
...
// Show emergency notification bubble
function showEmergencyNotification(emergencyData) {
    const container = document.getElementById('emergencyNotifications');
    
    const notification = document.createElement('div');
    notification.className = 'emergency-notification';
    notification.innerHTML = `
        <div class="emergency-notification-icon">
        ...
        <div class="emergency-notification-content">
            <div class="emergency-notification-title">🚨 Emergency Alert</div>
            <div class="emergency-notification-user">${emergencyData.userName}</div>
            <div class="emergency-notification-time">${new Date(emergencyData.timestamp).toLocaleTimeString()}</div>
        </div>
        <button class="emergency-notification-close" aria-label="Close">×</button>
    `;
    
    // Click to show full modal
    notification.addEventListener('click', (e) => {
        if (!e.target.classList.contains('emergency-notification-close')) {
    ...
    // Close button
    notification.querySelector('.emergency-notification-close').addEventListener('click', (e) => {
        e.stopPropagation();
...
function updatePosition(position) {
    ...
    // Update navigation route in real-time if active
    if (state.navigationMode && state.selectedUserId) {
...
// Initialize
console.log('Live NSWF Radar v4.5.1 initialized');
...
const alertAtPosition = document.getElementById('alertAtPosition');
const alertDelayed = document.getElementById('alertDelayed');
const alertComplete = document.getElementById('alertComplete');

// Get voice recording elements
...
// Alert/Ping Functions
function sendAlert(alertType) {
    if (!state.isSharing || !firebaseInitialized) return;
    ...
    // Update in Firebase
    database.ref(`users/${state.myId}/alert`).set(alertData);
    
    // Update local state
    state.currentAlert = alertData;
    
    // Show confirmation
    showNotification(`Alert sent: ${alertType}`, 'success');
}

function clearAlert() {
    if (!state.isSharing || !firebaseInitialized) return;
    
    database.ref(`users/${state.myId}/alert`).remove();
    state.currentAlert = null;
}

// Alert button handlers
alertAtPosition.addEventListener('click', () => sendAlert('At Position'));
alertDelayed.addEventListener('click', () => sendAlert('Delayed'));
alertComplete.addEventListener('click', () => sendAlert('Complete'));

// Messaging Functions
function toggleMessagingPanel() {
    messagingPanel.classList.toggle('open');
...
function sendTextMessage() {
    ...
    if (!text || !state.isSharing || !firebaseInitialized) return;
    ...
    const message = {
        ...
        content: text,
        senderId: state.myId,
        senderName: userNameInput.value || 'Unknown',
        senderType: state.myType,
        timestamp: Date.now()
    };
    ...
    state.messages.forEach(msg => {
        ...
        if (msg.type === 'text') {
            ...
            messageEl.innerHTML = `
                ...
                <div class="voice-message" data-url="${msg.voiceUrl}" data-duration="${msg.duration}">
...
function cleanupOldMessages() {
    ...
    database.ref('messages').orderByChild('timestamp').endAt(cutoffTime).once('value', (snapshot) => {
        snapshot.forEach((child) => {
            const msg = child.val();
            
            // Delete voice file from storage if exists
            if (msg.type === 'voice' && msg.voiceUrl) {
                // Extract path from URL and delete from Firebase Storage
                // Note: This requires Firebase Storage setup
                console.log('Cleaning up old voice message:', child.key);
            }
...
messagingToggle.addEventListener('click', toggleMessagingPanel);
closeMessaging.addEventListener('click', toggleMessagingPanel);
...
// Show messaging toggle when sharing
...
async function startVoiceRecording() {
    try {
        ...
        mediaRecorder.onstop = () => {
            ...
            // Upload to Firebase Storage (or convert to base64 for Firebase Realtime DB)
        ...
        // Start timer
        updateVoiceTimer();
        state.voiceRecordingInterval = setInterval(updateVoiceTimer, 100);
...
function updateVoiceTimer() {
    const elapsed = (Date.now() - state.voiceRecordingStartTime) / 1000;
    const remaining = MAX_RECORDING_TIME - elapsed;
    
    // Update timer display
    const seconds = Math.max(0, Math.floor(remaining));
    voiceTimer.textContent = `${seconds}s`;
    
    // Update progress bar
    const progress = (elapsed / MAX_RECORDING_TIME) * 100;
    voiceProgressBar.style.width = `${Math.min(100, progress)}%`;
    
    // Warning vibration at 2 seconds left
    if (seconds === 2 && navigator.vibrate) {
...
async function uploadVoiceMessage(audioBlob) {
    if (!firebaseInitialized) {
        showNotification('Cannot send voice message (offline)', 'error');
        return;
    }
    
    try {
        // Calculate duration
        const duration = Math.round((Date.now() - state.voiceRecordingStartTime) / 1000);
        
        // Convert to base64 for Firebase Realtime Database
        // (Alternative: Use Firebase Storage for larger files)
        ...
        reader.onloadend = () => {
            ...
            const message = {
                id: `msg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
                type: 'voice',
                voiceData: base64Audio, // Base64 encoded audio
                duration: duration,
                senderId: state.myId,
                senderName: userNameInput.value || 'Unknown',
                senderType: state.myType,
            ...
            showNotification(`Voice message sent (${duration}s)`, 'success');
        };
...
window.playVoiceMessage = function(button) {
    ...
    const voiceData = voiceMessage.dataset.url || voiceMessage.parentElement.querySelector('[data-voice-data]')?.dataset.voiceData;
    const duration = parseInt(voiceMessage.dataset.duration);
    const progressBar = voiceMessage.querySelector('.voice-waveform-progress');
    
    // Find voice data from message
    const messageEl = button.closest('.message-item');
    const messages = state.messages.filter(m => m.type === 'voice');
    const message = messages.find(m => m.duration === duration);
    
    if (!message || !message.voiceData) {
        showNotification('Voice message not available', 'error');
        return;
    }
    
    // Create audio element
    const audio = new Audio(message.voiceData);
    ...
    audio.play().catch(error => {
        ...
        showNotification('Cannot play voice message', 'error');
        button.innerHTML = `
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <polygon points="5 3 19 12 5 21 5 3"></polygon>
            </svg>
...
// Voice record button event listeners
voiceRecordBtn.addEventListener('click', startVoiceRecording);

// Click anywhere on modal to stop recording
voiceRecordModal.addEventListener('click', (e) => {
    if (e.target === voiceRecordModal) {
        stopVoiceRecording();
    }
});
...
// Clean up voice messages older than 24 hours
setInterval(() => {
    if (firebaseInitialized) {
        cleanupOldMessages();
    }
